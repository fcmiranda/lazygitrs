...
impl Gui {
    ...
    fn handle_key(&mut self, key: KeyEvent) -> Result<()> {
        ...
        if matches_key(key, &keybindings.universal.push_files) {
            controller::remotes::handle_key(
                self,
                key,
                &self.config.user_config.keybinding.clone(),
            )?;
            return Ok(());
        }
                    ...
                    } = &mut self.popup
                    {
                        match focus {
                            ...
                            popup::CommitInputFocus::Body => {
                                ...
                                let popup_inner = popup_width.saturating_sub(4) as usize;
                                let config_width =
                                    self.config.user_config.git.commit.auto_wrap_width;
                                let effective_width = if config_width > 0 {
                                    popup_inner.min(config_width)
                                } else {
                                    popup_inner
                            ...
                            if !text.is_empty() {
                                ...
                                } = editor
                                {
                                    ...
                                    if !body.is_empty() {
                                        body_textarea.insert_str(&body);
                                        let popup_width =
                                            (gui.layout.width * 60 / 100).min(60).max(30);
                                        let popup_inner = popup_width.saturating_sub(4) as usize;
                                        let config_width =
                                            gui.config.user_config.git.commit.auto_wrap_width;
                                        let wrap = if config_width > 0 {
                                            popup_inner.min(config_width)
                                        } else {
                                            popup_inner
